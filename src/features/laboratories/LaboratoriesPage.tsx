...
import { getLaboratoriesData } from "./api/laboratory";
import { useLaboratoryColumns } from "./hooks/useLaboratoryColumns";
...
export default function LaboratoriesPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["laboratories", page, debouncedSearch, filterQuery],
    queryFn: () => getLaboratoriesData(page, debouncedSearch, filterQuery),
  });

  const columns = useLaboratoryColumns();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-2">
        <div className="text-xl font-semibold">{t("Laboratories")}</div>
